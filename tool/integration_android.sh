...
# The state is read into a variable: `| grep -q` under pipefail can turn a
# match into exit 141 when grep stops reading early.
trust="$(adb -s "$serial" shell dumpsys trust)"
if [[ "$trust" == *deviceLocked=1* ]]; then
...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *DKV:CANCEL*)
      ( sleep 2; adb -s "$serial" shell input keyevent KEYCODE_BACK ) & ;;
  esac
done
